...
function Log-Computer {
    ...
    if(Test-Connection -ComputerName $computer -Quiet -TimeToLive 5) {
        ...
        if($path.EndsWith(".json")) { New-Log -Path $path -Data $payload } else { New-Log -Path ($path + "\$computer.json") -Data $payload }
...
function Query-Facility {
    ...
    if("$facility".IndexOf("(")){$facility = $facility.SubString(0, ("$facility".IndexOf("("))-1)}
